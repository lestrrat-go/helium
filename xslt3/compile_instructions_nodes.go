...
func (c *compiler) compileValueOf(ctx context.Context, elem *helium.Element) (*valueOfInst, error) {
	inst := &valueOfInst{}
	...
	if sep, ok := elem.GetAttribute("separator"); ok {
		avt, err := c.compileAVT(sep, c.nsBindings)
		if err != nil {
			return nil, err
		}
		inst.Separator = avt
...
func (c *compiler) compileElement(ctx context.Context, elem *helium.Element) (*elementInst, error) {
	...
	nameAVT, err := c.compileAVT(nameAttr, c.nsBindings)
	if err != nil {
		return nil, err
	}
	...
	if typeAttr != "" {
		if err := c.checkTypeAttrSchemaAware(ctx, "xsl:element", typeAttr); err != nil {
			return nil, err
		}
	...
	if nsAttr, hasNS := elem.GetAttribute("namespace"); hasNS {
		nsAVT, err := c.compileAVT(nsAttr, c.nsBindings)
		if err != nil {
			return nil, err
		}
...
func (c *compiler) compileAttribute(ctx context.Context, elem *helium.Element) (*attributeInst, error) {
	nameAttr := getAttr(elem, "name")
	if nameAttr == "" {
	...
	if typeAttr := getAttr(elem, "type"); typeAttr != "" {
		if err := c.checkTypeAttrSchemaAware(ctx, "xsl:attribute", typeAttr); err != nil {
			return nil, err
		}
		inst.TypeName = resolveXSDTypeName(typeAttr, c.nsBindings)
	}
	...
	if sep, hasSep := elem.GetAttribute("separator"); hasSep {
		sepAVT, err := c.compileAVT(sep, c.nsBindings)
		if err != nil {
			return nil, err
		}
...
func (c *compiler) compileComment(ctx context.Context, elem *helium.Element) (*commentInst, error) {
	inst := &commentInst{}

	selectAttr := getAttr(elem, "select")
	if selectAttr != "" {
		// XTSE0940: select and non-empty content are mutually exclusive.
		if err := c.validateEmptyElement(ctx, elem, "xsl:comment"); err != nil {
...
func (c *compiler) compilePI(ctx context.Context, elem *helium.Element) (*piInst, error) {
	nameAttr := getAttr(elem, "name")
	if nameAttr == "" {
	...
	if selectAttr != "" {
		// XTSE0940: select and non-empty content are mutually exclusive.
		if err := c.validateEmptyElement(ctx, elem, "xsl:processing-instruction"); err != nil {
...
func (c *compiler) compileCopy(ctx context.Context, elem *helium.Element) (*copyInst, error) {
	// Validate boolean attributes (including empty string)
	if inAttr, hasIn := elem.GetAttribute("inherit-namespaces"); hasIn {
	...
	if scn := getAttr(elem, "_copy-namespaces"); scn != "" {
		avt, err := c.compileAVT(scn, c.nsBindings)
		if err != nil {
			return nil, err
		}
	...
	if sin := getAttr(elem, "_inherit-namespaces"); sin != "" {
		avt, err := c.compileAVT(sin, c.nsBindings)
		if err != nil {
			return nil, err
		}
...
func (c *compiler) compileNumber(_ context.Context, elem *helium.Element) (*numberInst, error) {
	...
	if countAttr := getAttr(elem, "count"); countAttr != "" {
		p, err := compilePattern(countAttr, elem, c.xpathDefaultNS, c.hasXPathDefaultNS)
		if err != nil {
			return nil, err
		}
		inst.Count = p
	...
	if fromAttr := getAttr(elem, "from"); fromAttr != "" {
		p, err := compilePattern(fromAttr, elem, c.xpathDefaultNS, c.hasXPathDefaultNS)
		if err != nil {
			return nil, err
		}
		inst.From = p
	...
	if valueAttr := getAttr(elem, "value"); valueAttr != "" {
		...
		expr, err := c.compileXPath(valueAttr, c.nsBindings)
		if err != nil {
			return nil, err
		}
		inst.Value = expr
	...
	if gs := getAttr(elem, "grouping-separator"); gs != "" {
		avt, err := c.compileAVT(gs, c.nsBindings)
		if err != nil {
			return nil, err
		}
	...
	if gsz := getAttr(elem, "grouping-size"); gsz != "" {
		avt, err := c.compileAVT(gsz, c.nsBindings)
		if err != nil {
			return nil, err
		}
	...
	if ord := getAttr(elem, "ordinal"); ord != "" {
		avt, err := c.compileAVT(ord, c.nsBindings)
		if err != nil {
			return nil, err
		}
		inst.Ordinal = avt
	...
	if sa := getAttr(elem, "start-at"); sa != "" {
		avt, err := c.compileAVT(sa, c.nsBindings)
		if err != nil {
			return nil, err
		}
	...
	if lv := getAttr(elem, "letter-value"); lv != "" {
		avt, err := c.compileAVT(lv, c.nsBindings)
		if err != nil {
			return nil, err
		}
...
func (c *compiler) compileDocument(ctx context.Context, elem *helium.Element) (*documentInst, error) {
	inst := &documentInst{}
	if v := getAttr(elem, "validation"); v != "" {
...
func (c *compiler) compileLiteralResultElement(ctx context.Context, elem *helium.Element) (*literalResultElement, error) {
	// xsl:break / xsl:next-iteration not allowed inside element constructors.
	savedBreak := c.breakAllowed
	c.breakAllowed = false
	defer func() { c.breakAllowed = savedBreak }()
	...
	for _, attr := range elem.Attributes() {
		...
		avt, err := c.compileAVT(attr.Value(), c.nsBindings)
		if err != nil {
			return nil, err
		}
	...
	if len(c.stylesheet.namespaceAliases) > 0 {
		...
		if resultURI, resultPfx, ok := c.resolveNamespaceAlias(ctx, lre.Namespace); ok {
		...
		for _, attr := range lre.Attrs {
			...
			if resultURI, resultPfx, ok := c.resolveNamespaceAlias(ctx, attr.Namespace); ok {
		...
		for prefix, uri := range lre.Namespaces {
			if resultURI, resultPfx, ok := c.resolveNamespaceAlias(ctx, uri); ok {
				if resultPfx != "" {
