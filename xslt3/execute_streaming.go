...
func (ec *execContext) execSourceDocument(ctx context.Context, inst *sourceDocumentInst) error {
	...
	if !ok {
		...
		doc, err = parseExternalXML(ctx, ec.injectedParser(), data, resolvedURI, ec.allowExternalEntities(), ec.retrieveDocumentBytes, nil)
		if err != nil {
...
func (ec *execContext) loadMergeDocument(ctx context.Context, uri string, effectiveBaseURI string) (*helium.Document, error) {
	...
	doc, parseErr := parseExternalXML(ctx, ec.injectedParser(), data, resolvedURI, ec.allowExternalEntities(), ec.retrieveDocumentBytes, nil)
	if parseErr != nil {
