...
func evalVariable(goCtx context.Context, ec *evalContext, e VariableExpr) (Sequence, error) {
	if ec.vars != nil {
	...
	if ec.variableResolver != nil {
		if v, ok, err := ec.variableResolver.ResolveVariable(goCtx, e.Name); err != nil {
			return nil, err
		} else if ok {
			return enrichNodeItems(ec, v), nil
		}
...
func evalSequenceExpr(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, e SequenceExpr) (Sequence, error) {
	if len(e.Items) == 0 {
		return nil, nil //nolint:nilnil
	}
	var result ItemSlice
	for _, item := range e.Items {
		seq, err := evalFn(goCtx, ec, item)
		if err != nil {
			return nil, err
		}
...
func evalLocationPath(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, lp *LocationPath) (Sequence, error) {
	...
	for _, step := range lp.Steps {
		if len(step.Predicates) > 0 {
			nodes, err = evalStepWithPredicates(evalFn, goCtx, ec, nodes, step)
		} else {
			nodes, err = evalStepNoPredicates(goCtx, ec, nodes, step)
		}
		if err != nil {
			return nil, err
		}
...
func evalVMLocationPath(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, lp vmLocationPathExpr) (Sequence, error) {
	var nodes []helium.Node
	...
	for _, step := range lp.Steps {
		if len(step.Predicates) > 0 {
			nodes, err = evalVMStepWithPredicates(evalFn, goCtx, ec, nodes, step)
		} else {
			nodes, err = evalVMStepNoPredicates(goCtx, ec, nodes, step)
		}
		if err != nil {
			return nil, err
		}
...
func evalStepWithPredicates(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, nodes []helium.Node, step Step) ([]helium.Node, error) {
	...
	for _, n := range nodes {
		...
		if err := ec.countOps(goCtx, traversed); err != nil {
			return nil, err
		}
		for _, pred := range step.Predicates {
			matched, err = applyPredicate(evalFn, goCtx, ec, matched, pred)
			if err != nil {
				return nil, err
			}
		}
...
func evalStepNoPredicates(goCtx context.Context, ec *evalContext, nodes []helium.Node, step Step) ([]helium.Node, error) {
...
func evalVMStepWithPredicates(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, nodes []helium.Node, step vmLocationStep) ([]helium.Node, error) {
	allFiltered := make([]helium.Node, 0, len(nodes))
	for _, n := range nodes {
		matched, traversed, err := appendAxisNodeMatches(nil, ec, n, step.Axis, step.NodeTest)
		if err != nil {
		...
		for _, pred := range step.Predicates {
			matched, err = applyVMPredicate(evalFn, goCtx, ec, matched, pred)
			if err != nil {
				return nil, err
			}
		}
...
func applyVMPredicate(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, nodes []helium.Node, pred Expr) ([]helium.Node, error) {
	switch p := pred.(type) {
	...
	case vmAttributeEqualsStringPredicateExpr:
		return applyVMAttributeEqualsStringPredicate(evalFn, goCtx, ec, nodes, p)
	default:
		return applyPredicate(evalFn, goCtx, ec, nodes, pred)
	}
}
...
func applyVMAttributeEqualsStringPredicate(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, nodes []helium.Node, pred vmAttributeEqualsStringPredicateExpr) ([]helium.Node, error) {
	size := len(nodes)
	...
	for i, n := range nodes {
		...
		if !ok {
			...
			r, err := evalFn(goCtx, ec, pred.Fallback)
...
func evalVMStepNoPredicates(goCtx context.Context, ec *evalContext, nodes []helium.Node, step vmLocationStep) ([]helium.Node, error) {
	next := make([]helium.Node, 0, len(nodes))
	for _, n := range nodes {
		var traversed int
		var err error
...
func applyPredicate(evalFn exprEvaluator, goCtx context.Context, ec *evalContext, nodes []helium.Node, pred Expr) ([]helium.Node, error) {
	if err := ec.countOps(goCtx, len(nodes)); err != nil {
		return nil, err
	}
	size := len(nodes)
