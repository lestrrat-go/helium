...
func TestOutputEncoding(t *testing.T) {
	...
	// asserts that when OutputEncoding
	...
	t.Run("matches the emitted bytes", func(t *testing.T) {
	...
	// asserts a whitespace-padded OutputEncoding
	...
	t.Run("padded label is trimmed", func(t *testing.T) {
		src := []byte(`<?xml version="1.0" encoding="UTF-8"?><root>é</root>`)
		doc, err := helium.NewParser().Parse(t.Context(), src)
		require.NoError(t, err)
	...
	// asserts OutputEncoding affects the
	...
	t.Run("element fragment unchanged", func(t *testing.T) {
...
func TestOutputEncodingUSASCII(t *testing.T) {
	...
	t.Run("escapes non-ASCII", func(t *testing.T) {
	...
	// Covers U3–U7.
	t.Run("rejects DTD literals", func(t *testing.T) {
	...
	t.Run("allows an ASCII character-map replacement", func(t *testing.T) {
	...
	t.Run("rejects a non-ASCII character-map replacement", func(t *testing.T) {
		cases := map[string]string{
