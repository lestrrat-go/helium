...
func newEventEmitter(out io.Writer) sax.SAX2Handler {
	...
	s := sax.New()
	s.SetDocumentLocatorHandler = func(_ context.Context, _ sax.Context, loc sax.DocumentLocator) error {
		_, _ = fmt.Fprintf(out, "SAX.SetDocumentLocator()\n")
		return nil
	}
	s.AttributeDeclHandler = func(_ context.Context, _ sax.Context, elemName string, attrName string, typ int, deftype int, defvalue string, enum sax.Enumeration) error {
	...
	s.InternalSubsetHandler = func(_ context.Context, _ sax.Context, name, externalID, systemID string) error {
		_, _ = fmt.Fprintf(out, "SAX.InternalSubset(%s, %s, %s)\n", name, externalID, systemID)
		return nil
	}
	s.ReferenceHandler = func(_ context.Context, _ sax.Context, name string) error {
		_, _ = fmt.Fprintf(out, "SAX.Reference(%s)\n", name)
		return nil
	}
	s.GetEntityHandler = func(_ context.Context, _ sax.Context, name string) (sax.Entity, error) {
	...
	s.GetParameterEntityHandler = func(_ context.Context, _ sax.Context, name string) (sax.Entity, error) {
		_, _ = fmt.Fprintf(out, "SAX.ResolveEntity(%s)\n", name)
	...
	s.EntityDeclHandler = func(_ context.Context, ctxif sax.Context, name string, typ int, publicID string, systemID string, notation string) error {
	...
	s.ExternalSubsetHandler = func(_ context.Context, _ sax.Context, name, externalID, systemID string) error {
		_, _ = fmt.Fprintf(out, "SAX.ExternalSubset(%s, %s, %s)\n", name, externalID, systemID)
		return nil
	}
	s.ElementDeclHandler = func(_ context.Context, _ sax.Context, name string, typ int, content sax.ElementContent) error {
		_, _ = fmt.Fprintf(out, "SAX.ElementDecl(%s, %d, ...)\n", name, typ)
		return nil
	}
	s.StartDocumentHandler = func(_ context.Context, _ sax.Context) error {
		_, _ = fmt.Fprintf(out, "SAX.StartDocument()\n")
		return nil
	}
	s.EndDocumentHandler = func(_ context.Context, _ sax.Context) error {
		_, _ = fmt.Fprintf(out, "SAX.EndDocument()\n")
		return nil
	}
	s.CommentHandler = func(_ context.Context, _ sax.Context, data []byte) error {
		_, _ = fmt.Fprintf(out, "SAX.Comment(%s)\n", data)
		return nil
	}
	charHandler := func(name string, _ context.Context, _ sax.Context, data []byte) error {
		var output string
		if len(data) > 30 {
	...
	s.IgnorableWhitespaceHandler = func(_ context.Context, ctx sax.Context, data []byte) error {
		return charHandler("IgnorableWhitespace", context.TODO(), ctx, data)
	}
	s.CharactersHandler = func(_ context.Context, ctx sax.Context, data []byte) error {
		return charHandler("Characters", context.TODO(), ctx, data)
	}
	s.StartElementNSHandler = func(_ context.Context, _ sax.Context, localname, prefix, uri string, namespaces []sax.Namespace, attrs []sax.Attribute) error {
	...
	s.EndElementNSHandler = func(_ context.Context, _ sax.Context, localname, prefix, uri string) error {
