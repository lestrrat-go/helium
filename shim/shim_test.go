...
func TestUnmarshalTopLevelUnsupportedTypeMatchStdlib(t *testing.T) {
	input := []byte(`<root><k>v</k></root>`)

	stdOut := map[string]string{}
	shimOut := map[string]string{}
...
func TestUnmarshalAnyAttrMatchStdlib(t *testing.T) {
	type payload struct {
		ID    string        `xml:"id,attr"`
		Attrs []stdxml.Attr `xml:",any,attr"`
	}

	input := []byte(`<root id="42" a="x" b="y" c="z"/>`)
...
func TestUnmarshalTagPathPrefixConflictMatchStdlib(t *testing.T) {
	type payload struct {
		A string `xml:"a>b"`
		B string `xml:"a>b>c"`
	}

	input := []byte(`<root><a><b>x<c>y</c></b></a></root>`)
