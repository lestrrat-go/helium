...
func TestUnmarshalFeedStdlib(t *testing.T) {
	...
	if !reflect.DeepEqual(f, atomFeedStdlib) {
		t.Fatalf("have %#v\nwant %#v", f, atomFeedStdlib)
	}
}
...
var atomFeedStdlib = Feed{
	...
	Updated: ParseTimeStdlib("2009-10-04T01:35:58+00:00"),
	Author: Person{
		Name:     "rietveld<>",
...
`,
		...
		},
		{
			...
			Updated: ParseTimeStdlib("2009-10-03T23:02:17+00:00"),
			Author: Person{
				Name:     "email-address-removed",
				InnerXML: "<name>email-address-removed</name>",
			},
...
func TestUnmarshalWithoutNameTypeStdlib(t *testing.T) {
	...
	if x.Attr != okStdlib {
		t.Fatalf("have %v\nwant %v", x.Attr, okStdlib)
	}
}

func TestUnmarshalAttrStdlib(t *testing.T) {
...
func TestUnmarshalerStdlib(t *testing.T) {
...
func TestMalformedCommentStdlib(t *testing.T) {
	testData := []string{
		"<X><!-- a---></X>",
		"<X><!-- -- --></X>",
		"<X><!-- a--b --></X>",
...
const whitespaceValuesXMLStdlib = `
...
func TestUnmarshalWhitespaceValuesStdlib(t *testing.T) {
	v := WhitespaceValuesParent{}
	if err := Unmarshal([]byte(whitespaceValuesXMLStdlib), &v); err != nil {
...
const whitespaceAttrsXMLStdlib = `
...
// golang.org/issues/22146
func TestUnmarshalWhitespaceAttrsStdlib(t *testing.T) {
	v := WhitespaceAttrsParent{}
	if err := Unmarshal([]byte(whitespaceAttrsXMLStdlib), &v); err != nil {
...
func TestUnmarshalIntoNilStdlib(t *testing.T) {
	type T struct {
		A int `xml:"A"`
	}
