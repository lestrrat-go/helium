...
func TestAttribute(t *testing.T) {
	...
	// the Attribute node-interface methods.
...
func TestCreateAttribute(t *testing.T) {
	...
	// drives the stringToNodeList path inside
	// CreateAttribute by passing a value containing character and entity references.
	t.Run("value carrying an entity", func(t *testing.T) {
		doc := helium.NewDocument("1.0", "UTF-8", helium.StandaloneImplicitNo)
...
func TestSetAttribute(t *testing.T) {
	...
	t.Run("set", func(t *testing.T) {
		t.Run("rejects colon in name", func(t *testing.T) {
			...
			require.Error(t, elem.SetAttribute("xml:space", "preserve"))
			require.Error(t, elem.SetParsedAttribute("xml:lang", "en"))

			// Passing a proper local name should succeed.
			require.NoError(t, elem.SetAttribute("id", "123"))
			require.NoError(t, elem.SetParsedAttribute("lang", "en"))
		})

		// SetAttribute/SetAttributeNS store the value verbatim: a bare '&' is
		// kept as-is (no "entity was unterminated" error), and an already-escaped
		// "&amp;" is stored as the five literal characters. The writer then
		// escapes on the way out, so the verbatim '&' round-trips as "&amp;".
		t.Run("SetAttribute stores value verbatim", func(t *testing.T) {
			...
			require.NoError(t, elem.SetAttribute("b", "&amp;"))
			v, ok = elem.GetAttribute("b")
			require.True(t, ok)
			require.Equal(t, "&amp;", v)

			require.NoError(t, elem.SetAttributeNS("c", "A&B", ns))
			v, ok = elem.GetAttributeNS("c", "urn:x")
			require.True(t, ok)
			require.Equal(t, "A&B", v)

			require.NoError(t, elem.SetAttributeNS("e", "&amp;", ns))
			v, ok = elem.GetAttributeNS("e", "urn:x")
			require.True(t, ok)
			require.Equal(t, "&amp;", v)
		...
		// SetParsedAttribute/SetParsedAttributeNS parse the value as attribute-value
		// content: "&amp;" resolves to a single '&', and a malformed reference is an
		// error.
		t.Run("SetParsedAttribute resolves entity references", func(t *testing.T) {
			...
			v, ok := elem.GetAttribute("c")
			require.True(t, ok)
			require.Equal(t, "&", v)

			require.NoError(t, elem.SetParsedAttributeNS("d", "&amp;", ns))
			v, ok = elem.GetAttributeNS("d", "urn:x")
			require.True(t, ok)
			require.Equal(t, "&", v)

			// A malformed reference is rejected instead of being stored.
			require.Error(t, elem.SetParsedAttribute("bad", "A&B"))
			require.Error(t, elem.SetParsedAttributeNS("bad2", "A&B", ns))
		})
	...
	// Element.SetBooleanAttribute for both the
	...
	t.Run("namespaced duplicate", func(t *testing.T) {
	...
	t.Run("namespaced duplicate under different prefixes", func(t *testing.T) {
		t.Run("same namespace URI via different prefixes replaces in place", func(t *testing.T) {
			...
			require.NoError(t, e.SetAttributeNS("a", "first", ns1))
			require.NoError(t, e.SetAttributeNS("a", "second", ns2))
...
func TestGetAttribute(t *testing.T) {
	...
	t.Run("namespaced node", func(t *testing.T) {
		doc := helium.NewDefaultDocument()
		e, err := doc.CreateElement("root")
		require.NoError(t, err)
...
func TestAttributeNamespaces(t *testing.T) {
	...
	t.Run("well-formedness constraints", func(t *testing.T) {
	...
	t.Run("DTD default does not override an explicit declaration", func(t *testing.T) {
...
func TestAttributeListRepair(t *testing.T) {
	t.Parallel()
	...
	// matched.
	t.Run("AddSibling move repairs the property list", func(t *testing.T) {
		cases := []struct {
			name string
	...
	// detached.
	t.Run("Replace in the property list", func(t *testing.T) {
		cases := []struct {
			name string
	...
	// an attribute
	// forced into the normal child list (via the no-preflight append, NOT the property
	...
	t.Run("AddChild move repairs the child list", func(t *testing.T) {
		cases := []struct {
			name string
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				require.NoError(t, helium.UnsafeAppendChildForTesting(src, a), "add attribute a as child")
				require.NoError(t, helium.UnsafeAppendChildForTesting(src, b), "add attribute b as child")
				require.NoError(t, helium.UnsafeAppendChildForTesting(src, c), "add attribute c as child")
	...
	t.Run("AddSibling move repairs the child list", func(t *testing.T) {
		doc := helium.NewDefaultDocument()
		src, err := doc.CreateElement("src")
		require.NoError(t, err)
	...
	// attribute.
	t.Run("Replace repairs the child list", func(t *testing.T) {
		cases := []struct {
			name string
