...
func (d *writeSession) dumpAttributeDecl(out io.Writer, n *AttributeDecl) error {
	...
	if n.defvalue != "" {
		d.writeString(out, ` "`)
		d.check(escapeAttrValue(out, []byte(n.defvalue), d.escapeNonASCII, d.asciiOutput, d.rejectInvalidChars, d.xml11, nil))
		d.writeString(out, `"`)
	}
	d.writeString(out, ">\n")
	return d.err
...
func (d *writeSession) dumpNs(out io.Writer, ns *Namespace) error {
	...
	if err := escapeAttrValue(out, []byte(ns.href), d.escapeNonASCII, d.asciiOutput, d.rejectInvalidChars, d.xml11, nil); err != nil {
		return err
	}
	if _, err := io.WriteString(out, `"`); err != nil {
		return err
