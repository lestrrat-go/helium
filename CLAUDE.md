...
Implemented in 1.1 mode so far: the 1.1-only lexical forms (`+INF` for xs:double/xs:float; year `0000` on the date types — both gated in `internal/xsd/value` via a `value.Version` argument; relaxng is pinned to `value.Version10`); the 1.1 built-in datatypes (xs:dateTimeStamp, xs:dayTimeDuration, xs:yearMonthDuration, xs:anyAtomicType, xs:error); **UPA weakening** (`check_upa.go` `entriesOverlap`: in 1.1 an element competing with a wildcard is not a cos-nonambig violation — the element wins. Element-over-wildcard precedence is now ENFORCED at validation for the CHOICE case (`validate_elem.go` `matchChoice`/`tryMatchChoice`, gated on `vc.version == Version11`): a branch that consumes the current child via an element leaf AS ITS FIRST CONSUMING TERM takes precedence over one that would consume it via a wildcard, regardless of declaration order or nesting. The classifier (`particleConsumesViaElement` → `particleFirstConsumerKind`/`groupFirstConsumerKind`) is PATH-AWARE: it respects compositor order, occurrences, and emptiable prefixes (reusing `particleEmptiable`), so a LEADING wildcard inside a sequence (e.g. `sequence(any skip, element a)`) is correctly classified as wildcard-first and does NOT win element precedence even though a later element leaf in the same group also matches — only bounded first-consumer determination, no backtracking. A `skip` wildcard declared before a typed element (directly or as a leading nested term) thus no longer steals the element's child and false-accepts an invalid value. Selection is COMMIT-NO-FALLBACK: once any branch is element-first for the current child the choice MUST use an element-first branch and never falls back to a wildcard branch, even if the chosen element branch then fails structurally (a later required term missing, e.g. `choice(sequence(a:int, b:int), any skip)` with only `<a>`) or by content. The SEQUENCE case is a remaining limitation — sequence matching is position-based/greedy, so a minOccurs=0 wildcard preceding an element can still consume the element's child); **xs:assert on complex types** (`assert.go`: parsed from complexType/restriction/extension, pre-compiled via xpath3, evaluated against the element after content validation — EBV false → invalid; inherited down the base chain); and **conditional type assignment** (`alternative.go`: `<xs:alternative>` on an element declaration selects the governing type via the first @test that holds against the element, else a testless default, else the declared type; xsi:type still takes precedence; applied at the root and all three per-child match sites. The governing type may be a named `@type` OR an **inline anonymous `<xs:complexType>`/`<xs:simpleType>`** (`parseInlineAlternativeType`, compiled via the same path as an inline element/attribute type, refs resolved in resolveRefs). An alternative selecting **`xs:error`** makes the element invalid (empty value space): both `validateElementContent` AND `validateNilledElement` reject any element whose governing type is `isErrorType` (so `xsi:nil="true"` cannot bypass it). **`xpathDefaultNamespace`** (local on `<xs:alternative>`, else inherited from `<xs:schema>` via `compiler.xpathDefaultNS`) sets the XPath default element namespace for @test through the `""` namespace binding; the `##targetNamespace`/`##defaultNamespace`/`##local` keywords resolve against the alternative's context (`effectiveXPathDefaultNS`). The default is `##local`: `parseTypeAlternative` DELETES the `""` binding `collectNSContext` derives from the schema document's `xmlns="…"`, so an unprefixed name test matches only no-namespace names unless `xpathDefaultNamespace` re-adds `""`. CTA is a property of the referenced GLOBAL declaration, so `applyTypeAlternatives` falls back to the global's alternatives for an `<xs:element ref>` particle, mirroring idcHostDecl); and **open content** (`opencontent.go`: `<xs:openContent>` interleave/suffix — interleave removes children whose names aren't declared and which match the wildcard, then matches the declared model on the rest; suffix matches the declared model then requires every trailing child to match the wildcard; declared-named children always go through the model per weak-wildcard precedence). **Not yet implemented** (planned, gated behind the same toggle): the **xs:assertion simple-type facet** (needs `$value` bound to the typed atomic), **xs:defaultOpenContent** (schema-level default), `xs:all` member `maxOccurs>1` and wildcards-in-all, wildcard `notNamespace`/`notQName`, `xs:override`, the general user-declarable `explicitTimezone` facet, and `vc:typeAvailable`/`vc:facetAvailable` conditional pruning. Do NOT assume these work in 1.1 mode yet. **Inherited attributes** ARE implemented (`inherited_attrs.go`): `inheritable="true"` on an attribute declaration/use (the use's value wins over the referenced global decl; the value is whitespace-collapsed xs:boolean, and a non-boolean lexical is a schema error) marks the attribute, and the validation walk records each matched inheritable instance attribute (`validationContext.attrInheritable`) so a descendant's CTA `@test` resolves it as an inherited attribute (`inheritedAttributes`: nearest ancestor wins; a non-inheritable attribute does NOT mask an inheritable ancestor). The **CTA XPath context** is now spec-correct: `ctaContextNode` builds a detached element copy carrying the element's name, in-scope namespaces, and own + inherited attributes but NO children or parent (XSD 1.1 §3.13), evaluated with `position()=1`/`last()=1`, an empty `fn:collection()` resolver, and the schema's base URI — so `empty(..)`, `string()=''`, `. is root()`, `instance of element(*, xs:untyped)` and inherited-attribute tests behave per spec. The CTA context retains `xml:*` attributes (`xml:lang`/`xml:base`/`xml:space`) — `ctaContextNode` filters with `isNamespaceDeclAttr` (namespace declarations only), NOT `isSpecialAttr` — so a test can discriminate on `@xml:lang`. Inheritance also covers attributes admitted through an `xs:anyAttribute` wildcard (`validateWildcardAttr`) and defaulted/fixed attributes inserted at validation — both recorded in `attrInheritable`. Three CTA schema-validity rules are enforced: a restriction may not change an attribute use's `inheritable` (`checkRestrictionAttrs`); each alternative's type must be validly substitutable for the element's declared type (`checkAltSubstitutability`/`isValidlySubstitutable`): an alternative type T is valid iff it is `xs:error`, OR `strictBuiltinAwareDerivedFrom(T, declared)` holds (genuine Type Derivation OK — identity, user restriction/extension chains via `isDerivedFrom`, the built-in simple-type hierarchy, and the xs:anySimpleType simple-content rule), OR the declared type is a union and T is validly substitutable for some member type RECURSIVELY. There is NO permissive simple-vs-simple fallback — an unrelated simple alternative (e.g. `xs:string` for a user `SmallInt`, or for `union(SmallInt, xs:boolean)`) is rejected, as is any unrelated/complex alternative; a genuine user derivation is caught by `isDerivedFrom` (user BaseType chains are linked by resolveRefs). The complex-vs-simple discriminator is `TypeDef.IsComplex` (set by `parseComplexType`/built-in xs:anyType), NOT `ContentType`, since a complex type with `<xs:simpleContent>` also has `ContentType == ContentTypeSimple`. An `xs:alternative` must have exactly ONE governing-type source (`@type` XOR a single inline type — both, or two inline types, is a schema error; presence is tested with `hasAttr`, so a present-but-empty `@type=""` counts as a malformed type source — `@type=""` plus inline is a conflict, a bare `@type=""` is an invalid QName). `fn:static-base-uri()` in a CTA test is sourced from the schema document's URL (`doc.URL()`, else the `CompileFile` path) via `compiler.schemaBaseURI` — NEVER the diagnostic label — so it resolves whenever the caller sets the document URL. `xpathDefaultNamespace` is whitespace-collapsed before its `##keyword`/URI value is interpreted. (Limitation: xs:assert's test still runs against the live element, not the detached CTA-style context.)
