...
(`FOTY0012`, `FOTY0013`, `FORG0001`, …) propagates unchanged, with no collapse into
`XPTY0004`. The same `coerceToSequenceTypeE` propagation applies
...
`xmlns:p=""` as an UNDECLARATION that leaves `p` unbound, with no binding to the
...
raises `SEPM0016` at dispatch, so it can never silently fall through to xml. The
