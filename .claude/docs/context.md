...
- `xpath3` does NOT carry evaluation config on `context.Context`. It uses fluent setters on the value-type `*Evaluator` (`Namespaces`, `Variables`, `OpLimit`, `DefaultLanguage`, `URIResolver`, etc.); `Evaluate`/`EvaluateReuse` accept a `context.Context` for cancellation/deadlines only. See `xpath3-api.md`.
