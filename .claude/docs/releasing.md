...
Helium releases are **dispatch-driven**, not tag-triggered. The conformance
gate runs *before* the tag exists, so a failed conformance run never leaves a
public tag or partial release behind.
...
4. On green, the `release` job waits for **environment approval** (`release`
   environment, maintainer reviewer, pinned to `main`). Approve it in the run.
...
**Bumping the pin:** the nightly `conformance.yml` run tracks `helium-w3c-tests@main`
