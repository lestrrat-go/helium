...
- **`ErrContentSizeExceeded`** (exported) — returned from `parse()` when an over-cap construct blows the `Parser.MaxContentSize` hard cap. Wrapped with a descriptive prefix via `fmt.Errorf("... : %w", ErrContentSizeExceeded)`; callers match with `errors.Is`. The error reaches `parse()`'s caller via one of two surfacing paths, both checked at the top of (and after) the main loop: a sub-parser sets `parser.fatalErr` for an in-band content overrun, OR — on the streaming `ParseReader`/push path — the over-cap deferred-encoding reader sets a sticky `capErr` whose bytes propagate up the cursor as `p.cur.Err()`. Returned in these cases:
  ...
  - An over-cap leading-whitespace deferral (`deferPendingWS`) or an over-cap attribute value (`parseAttributeValue`) before its significance/terminator is known. (Sets `parser.fatalErr`.)
