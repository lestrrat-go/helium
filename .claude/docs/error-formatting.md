...
`ErrReferenceNotFound` and `ErrAmbiguousReference` are the two same-document `ds:RetrievalMethod` resolution sentinels (`reference.go` `resolveSameDocument`). `ErrReferenceNotFound` covers a URI that is not one of the four same-document forms at all — an external URI included, with no setting that lifts the refusal, since xmlenc-core1 §3.5 mandates only the same-document form — and a same-document URI matching no element. `ErrAmbiguousReference` covers a URI matching MORE than one element: resolution collects every match and refuses on a duplicate rather than taking the first, because an injected duplicate `Id` would otherwise choose which key the recipient unwraps (XML Signature Wrapping applied to encryption). Both wrap via `%w` and are matched with `errors.Is`. They are raised while the document is READ, so they precede the `Decryptor.SessionKey` early return: a pre-shared session key does not decrypt past a refused reference. A `ds:RetrievalMethod` whose `Type` this package does not implement is stepped over before any URI is looked at and so reaches neither sentinel; a `Type` of `TypeEncryptedKey` that resolves to a non-`EncryptedKey` element is `ErrMalformedEncrypted`, not a reference error.

Block encryption and decryption failures wrap `ErrEncryptionFailed` or `ErrDecryptionFailed` as appropriate. Unsupported block-algorithm URI failures preserve `*UnsupportedAlgorithmError` in the chain, so callers can match the operation sentinel with `errors.Is` and inspect the algorithm with `errors.As`.
