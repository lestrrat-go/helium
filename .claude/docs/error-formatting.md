...
- **`ErrElementDeclNotFound`** (`errors.go`) — sentinel returned by `Document.IsMixedElement` when the internal subset has no element declaration for the given name (or the document has no internal subset). Match with `errors.Is`
