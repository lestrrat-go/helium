...
- **Writer structural-serialization sentinels** (`errors.go`) — `ErrWriterReservedElementName`, `ErrWriterReservedAttributeName`, `ErrWriterReservedNamespacePrefix`, `ErrWriterInvalidElementName`, `ErrWriterInvalidAttributeName`, `ErrWriterInvalidNamespacePrefix`, `ErrWriterUnboundNamespacePrefix`, `ErrWriterInvalidName`, `ErrWriterInvalidComment`, `ErrWriterInvalidPITarget`, `ErrWriterInvalidPIContent`, and `ErrWriterInvalidDTDNode`. They flag a DOM node that cannot be serialized into well-formed XML. `ErrWriterUnboundNamespacePrefix` covers an element or attribute QName with a non-empty prefix but no namespace URI; the reserved `xml` prefix is implicitly bound. `ErrWriterInvalidName` covers DTD and entity-reference names that fail the XML Name grammar (`checkVerbatimName`). The original message wraps the sentinel with `%w`, so callers can use `errors.Is`; the checks preserve an earlier I/O error. Full list in `packages.md`
