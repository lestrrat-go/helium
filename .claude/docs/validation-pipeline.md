...
<!-- The XSD-version paragraph above does not name `inheritRestrictionAttrs10`, and there is
     no restriction-only XSD 1.0 attribute pass. `git grep -n inheritRestrictionAttrs10`
     matches only lines of this comment; no file under `xsd/` and no other doc contains the
     identifier. The paragraph names `finalizeAttrUses10` and states the pass runs
     topologically across BOTH extension and restriction, which is what
     `compiler.finalizeAttrUses10` implements (it recurses through `BaseType` whatever its
     derivation kind) and what the non-Version11 branch of `compiler.resolveRefs` calls over
     `extensionTypes` + `restrictionTypes`. -->
