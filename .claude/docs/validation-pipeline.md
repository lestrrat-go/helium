...
leaf type, descending past every intermediate union. On an ATOMIC restriction the range facets
...
`ok=false`, leaving the range facet INAPPLICABLE, so no comparison is forced.
...
schema error, caught before it could become a runtime no-op. `explicitTimezone` is accepted only on the
...
(`\i`, `\c`, `\p{Is...}` blocks) are enforced in full;
...
- **Host declaration resolution** (`idcHostDecl`): the declaration whose IDCs apply
  ...
  declared on a LOCAL element buried in a content model is EVALUATED. `lookupElemDecl`
  finds only globals, so the record is what reaches a local one. The ref fallback exists
...
- For elements with IDCs (xs:unique, xs:key, xs:keyref):
  ...
  - Field cardinality (cvc-identity-constraint.3): for each selected node every
    ...
    to a node-set with more than one member.`), so the first node is never
    silently used.
  ...
  - Field type resolution (`resolveElemType`) consults the `actualElemType` map
    ...
    xs:integer value space, so the pair is correctly rejected as a duplicate. These
...
reported as a fatal error. `resolveIDCNameQName`
...
names/libraries fail, and raw equality never stands in for a match.
...
(including on QName/NOTATION), so no facet
silently accepts every value (`minLength="-1"`) and no bound is leniently
trimmed. At validation `validateWithParams` parses the bound
...
in full, with no false rejection. `checkDataFacets` compiles the pattern once at compile
...
facets (`value.IsFloatNaN`), so the facet FAILS and a NaN never slips through. The
