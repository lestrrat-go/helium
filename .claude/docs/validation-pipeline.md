...
**XSD version (1.0 default, 1.1 opt-in):** `Compiler.Version(xsd.Version11)` selects 1.1; an explicit call wins, else `resolveVersion` (`compile.go`) reads a `vc:minVersion="1.1"` hint (ns `lexicon.NamespaceXSDVersioning`) off the root `<xs:schema>`; default is `Version10`. The resolved `Version` is stored on `compiler.version`, propagated to the import sub-compiler (`compile_imports.go`), and frozen onto `Schema.version`; `validationContext.version` is seeded from it (`newValidationContext`) and from `c.version` for the compile-time facet sub-contexts (`check_facets.go`). The lexical gate lives in `internal/xsd/value.ValidateBuiltin(value, builtinLocal, version)` — `validateFloat` selects `floatRegex10` (no `+INF`) for 1.0; the date validators reject year `0000` for 1.0 (`yearForbiddenInXSD10`). The value package's comparison/canonicalization guards pass `value.Version11` (they run on already-validated values). relaxng pins its `ValidateBuiltin` calls to `value.Version10`. xsd threads the version into `validateBuiltinValue` via `vc.version`; `fixedUnionActiveMember` now takes a `version Version` param threaded from every caller (`fixedUnionMatches`/`crossMemberValueEqual(Depth)`/`fixedValueMatches`/`fixedListMatches` carry it, runtime callers pass `vc.version`, and the compile-time restriction-derivation chain in `restriction_particle.go` passes `c.version`), so the throwaway context it builds applies the schema's real version — a 1.1-only lexical form (e.g. `+INF`) inside a union fixed-value or enumeration literal is accepted in 1.1 mode. Two throwaway-context free funcs remain on the `Version10` default — `typeAcceptsValue` (`validate_idc.go`, IDC field active-member) and `TypeDef.Validate` (standalone validation) — a documented Phase-1 gap for 1.1-only lexical forms on those paths. The 1.1 built-in datatypes (xs:dateTimeStamp, xs:dayTimeDuration, xs:yearMonthDuration, xs:anyAtomicType, xs:error) are registered only in 1.1 mode by `registerBuiltinTypes11` (`compile.go`), with `BaseType` linked to their primitives; `Compare`/`CanonicalKey` route dateTimeStamp→dateTime and the durations→duration. **UPA weakening** is implemented: `entriesOverlap` (`check_upa.go`) takes the automaton's `schema.version` and, in 1.1, treats an element-vs-wildcard pair as non-overlapping (the element wins), so such a content model compiles; element-vs-element and wildcard-vs-wildcard are unchanged. Element-over-wildcard precedence is now ENFORCED at validation for the CHOICE case (`validate_elem.go` `matchChoice`'s `matchAt`/`matchOnce` and `tryMatchChoice`'s `scanOnce`, both gated on `vc.version == Version11`): a branch that consumes the current child via an element leaf AS ITS FIRST CONSUMING TERM is tried before any branch that would consume it via a wildcard, regardless of declaration order or nesting, so a `skip`/`lax` wildcard declared BEFORE a typed element no longer steals the element's child and false-accepts an invalid value (`<a>not-int</a>` against `xs:int`). The selection is COMMIT-NO-FALLBACK: if ANY branch is element-first for the current child, the choice MUST use an element-first branch and MUST NOT fall back to a wildcard branch — even when the chosen element branch then fails structurally (a later required term is absent, e.g. `choice(sequence(a:int, b:int), any skip)` with only `<a>`) or by content. `matchOnce` first tries each element-first branch via `matchAt`; if none matches fully it commits to the first element-first branch and surfaces its real `matchParticle` failure (recording the genuine content/missing-element error) instead of entering the wildcard pass, and `tryMatchChoice`'s `scanOnce` mirrors this — once an element-first branch exists for the child it returns that branch's success/failure and never falls back — so lookahead and real passes agree. Only when NO branch is element-first for the child may the wildcard pass run. This stays bounded (the commit is to the first element-first branch in declaration order; no backtracking). The classifier `particleConsumesViaElement` delegates to `particleFirstConsumerKind`/`groupFirstConsumerKind` (consumerKind = element/wildcard/none), which is PATH-AWARE: it walks sequence members in order respecting occurrences and emptiable prefixes (reusing `restriction_particle.go`'s `particleEmptiable`), so a LEADING wildcard inside a sequence — e.g. `sequence(any skip, element a)` — is classified wildcard-first and does NOT win element precedence even though a later element leaf in the same group also matches the child; choice/all are element-first iff some member is. This is bounded first-consumer determination, not full backtracking. XSD 1.0 keeps pure declaration-order matching (byte-identical path). The SEQUENCE case is a remaining limitation noted in `matchSequence` — sequence matching is position-based/greedy with no backtracking, so a minOccurs=0 wildcard preceding an element can still consume the element's child. **Assertions (xs:assert on complex types)** are implemented (`assert.go`): `parseAssert` reads `@test` from `<xs:assert>` children of complexType/restriction/extension (1.1 only), captures the in-scope namespaces (`collectNSContext`), and pre-compiles via `xpath3.NewCompiler().Compile` (a missing/malformed test is a fatal schema error, mirroring IDC); the result is stored on `TypeDef.Assertions` (the `Assertion` struct mirrors `IDConstraint`). `validateElementContent` extracts the content switch into `validateContentByType` and, after attributes+content validate, calls `checkAssertions`, which walks the type's base chain and evaluates each test with the element as context node via `xpath3.NewEvaluator(...).Namespaces(a.Namespaces).Evaluate(ctx, expr, elem)` then `xpath3.EBV` — false → validity error. StrictPrefixes is intentionally NOT set so xs:/fn: keep default bindings. Known limitation: the test runs against the element in the full document (can navigate to ancestors). **Conditional type assignment (xs:alternative)** is implemented (`alternative.go`): `parseTypeAlternatives` reads the `<xs:alternative>` children of an xs:element (1.1 only) in order — each carries an optional `@test` (absent = unconditional default) pre-compiled via xpath3 and a `@type` reference registered on `compiler.altTypeRefs` and resolved by `resolveAltTypeRefs` in `resolveRefs`; results are stored on `ElementDecl.Alternatives` (the `TypeAlternative` struct). At validation `applyTypeAlternatives` runs at all four type-selection sites (root `validateRootElement`, and `matchAll`/`matchElementParticle`/`matchWildcardParticle` in `validate_elem.go`) between `effectiveDeclType` and `resolveXsiType`: when no xsi:type is present (`elementHasXsiType`) it returns the type of the first alternative whose `@test` EBV is true (or the testless default), else the declared type — so xsi:type still wins; a failing alternative test is treated as not-matched. The governing type may be a named `@type` OR an **inline anonymous `<xs:complexType>`/`<xs:simpleType>`** (`parseInlineAlternativeType`, compiled through the same path as an inline element/attribute type so its refs resolve in resolveRefs). An alternative selecting **`xs:error`** makes the element invalid (empty value space): both `validateElementContent` AND `validateNilledElement` reject any element whose governing type satisfies `isErrorType` (the nilled-element path is guarded too, so `xsi:nil="true"` cannot bypass an xs:error governing type). **`xpathDefaultNamespace`** (local on the `<xs:alternative>`, else inherited from `<xs:schema>` via `compiler.xpathDefaultNS`/`xpathDefaultNSSet`) supplies the XPath default element namespace for `@test` by writing the `""` entry of the alternative's namespace map (`effectiveXPathDefaultNS`); `##targetNamespace`/`##defaultNamespace`/`##local` resolve against the alternative's context. The default is `##local` (no default element namespace): `parseTypeAlternative` DELETES the `""` binding that `collectNSContext` derives from the schema document's `xmlns="…"`, so an unprefixed name test (e.g. `self::root`) matches only no-namespace names unless `xpathDefaultNamespace` re-adds `""`. Because CTA is a property of the referenced GLOBAL declaration, `applyTypeAlternatives` falls back to the global element's alternatives for an `<xs:element ref>` particle (whose `ElementDecl` is a ref and does not carry the type table), mirroring `idcHostDecl`. **CTA XPath context (`inherited_attrs.go`):** the `@test` is evaluated against a SPEC-CORRECT detached context node (`ctaContextNode`) — a copy of the element with its name, in-scope namespaces, and own + inherited attributes but NO children or parent (XSD 1.1 §3.13) — configured with `position()=1`/`last()=1`, an empty `fn:collection()` resolver, and the schema base URI, so `empty(..)`, `string()=''`, `. is root()`, `instance of element(*, xs:untyped)` and inherited-attribute tests behave per spec. Own attributes are filtered by `isNamespaceDeclAttr` (namespace DECLARATIONS only), NOT `isSpecialAttr`, so `xml:*` attributes (`xml:lang`/`xml:base`/`xml:space`) are RETAINED and can drive a CTA test. **Inherited attributes:** `inheritable="true"` on an attribute decl/use (the use's value wins over the global decl; whitespace-collapsed xs:boolean, non-boolean = schema error) is parsed in `readAttributeUseDecl`/`parseAttributeUse`; each matched inheritable instance attribute is recorded in `validationContext.attrInheritable` during the top-down walk — including attributes admitted through an `xs:anyAttribute` wildcard (`validateWildcardAttr`) and defaulted/fixed attributes inserted at validation — and `inheritedAttributes` collects them for a descendant (nearest ancestor wins; a non-inheritable attribute does NOT mask an inheritable ancestor). **CTA schema-validity:** `checkRestrictionAttrs` rejects a restriction that changes an attribute use's `inheritable`; `checkAltSubstitutability`/`isValidlySubstitutable` rejects an alternative type not validly substitutable for the element's declared type. An alternative type T is valid iff it is `xs:error`, OR `strictBuiltinAwareDerivedFrom(T, declared)` holds (genuine Type Derivation OK — identity, user restriction/extension chains via `isDerivedFrom`, the built-in simple-type hierarchy, and the xs:anySimpleType simple-content rule), OR the declared type is a union and T is validly substitutable for some member type RECURSIVELY. There is NO permissive simple-vs-simple fallback: an unrelated simple alternative (e.g. `xs:string` for a user `SmallInt`, or for `union(SmallInt, xs:boolean)`) is rejected, as is any unrelated/complex alternative — a genuine user derivation is caught by `isDerivedFrom` (user BaseType chains are linked by resolveRefs). The complex-vs-simple discriminator is `TypeDef.IsComplex` (set by `parseComplexType` and for built-in xs:anyType), NOT `ContentType`, because a complex type with `<xs:simpleContent>` also carries `ContentType == ContentTypeSimple`. `parseTypeAlternative` rejects an `xs:alternative` with more than one governing-type source (`@type` plus an inline type, or two inline types) — source presence is tested with `hasAttr`, so a present-but-empty `@type=""` counts as a (malformed) type source: `@type=""` plus an inline type is a conflict, and a bare `@type=""` is reported as an invalid QName. **Static base URI:** `fn:static-base-uri()` in a CTA test is sourced from `compiler.schemaBaseURI` — the document URL (`doc.URL()`, else the `CompileFile` path), NEVER the diagnostic label — so it resolves whenever the caller sets the document URL. `xpathDefaultNamespace` is whitespace-collapsed before its `##keyword`/URI value is interpreted. **Per-document CTA static context:** `schemaBaseURI`/`xpathDefaultNS`/`xpathDefaultNSSet` are PER schema document — `loadInclude`/`loadRedefine` save/restore them around parsing (set from the included/redefined document + path, mirroring the form/default save-restore), and the import sub-compiler seeds them from the imported document, so an alternative in an included/imported file sees ITS file's base URI and xpathDefaultNamespace. `loadImport` merges `impC.ctaElems` and `impC.altTypeRefs` into the parent so imported alternatives are resolved (`resolveAltTypeRefs`) and substitutability-checked. `checkAltSubstitutability` resolves the element's declared type via `effectiveDeclType` (so a substitution-group member with no explicit type is checked against its head's type, not nil), and `ctaContextNode` builds the synthetic node with `elem.LocalName()` before `SetActiveNamespace` so a name test matches a prefixed instance element. **Open content (xs:openContent)** is implemented (`opencontent.go`): `parseOpenContent` reads `<xs:openContent>` (mode interleave/suffix/none; the `<xs:any>` child becomes the wildcard) on complexType/restriction/extension into `TypeDef.OpenContent`. `validateContentByType` routes the element-only/mixed path through `validateContentModelOpen` when present: **suffix** matches the declared model (`matchContentModel`, factored out of `validateContentModelTop`) then requires every trailing child to match the wildcard (a trailing declared-named child is "not expected"); **interleave** partitions children — those whose expanded name is NOT in `collectModelElementNames(mg)` and which match the wildcard are the open set, the rest must satisfy the declared model. Open children are validated via `matchWildcardParticle` (lax/strict/skip). Weak-wildcard precedence holds in both modes (a declared-named element never becomes open content). Remaining 1.1 structural features (the **xs:assertion simple-type facet** needing `$value` typed-atomic binding, **xs:defaultOpenContent** schema-level default, `xs:all` member maxOccurs>1 / wildcards-in-all, wildcard notNamespace/notQName, xs:override) are NOT yet implemented.
