...
- `DeclareNamespace(prefix, uri)` / `AddNamespaceDecl(ns)` — keep AT MOST ONE declaration per prefix in `nsDefs` and NEVER touch the node's active namespace (`n.ns`) or expanded name. `DeclareNamespace` allocates a fresh `*Namespace`; `AddNamespaceDecl` attaches the caller's existing object (no alloc), so a tree-builder can reuse one Namespace as both a declaration and an element's active ns. Both apply the same rule keyed on the prefix: (1) no existing entry → append; (2) existing entry, same URI → no-op; (3) existing entry, different URI, prefix NOT in use by `n.ns` or any attribute's `ns` → COLLAPSE (replace the single slot; the old object is left unmutated, so an aliasing `n.ns`/`attr.ns` or a caller-retained handle is unaffected); (4) existing entry, different URI, prefix IS in use by `n.ns` or an attribute → genuine conflict, tree unchanged (`DeclareNamespace` returns a `%w`-wrapped `ErrInvalidOperation`; `AddNamespaceDecl` is void so it silently declines). Prefix GENERATION for a conflicting holder is a serialize/reconcile concern (`writer.go` `reconcileNamespaces`), never in these mutators. `AddNamespaceDecl(nil)` appends nil verbatim (dedup skipped) for callers deliberately corrupting a node.
