...
The `DTD.Add*` builders (`AddEntity`/`AddNotation`/`AddElementDecl`/`AddAttributeDecl`) build a declaration from public parameters, register it in the lookup table, AND link it into the DTD child list so it serializes. `AddAttributeDecl(elem, name, atype, def, defvalue, enumValues)` is the `<!ATTLIST>` counterpart; the low-level table-only `registerAttribute` is unexported. `AddAttributeDecl` validates every parameter against the same rules the parser's `<!ATTLIST>` path and the declaration-time validity constraints enforce — `validateAttrDeclIdentity` for the identity parts (element name, attribute QName), then a duplicate check, then `validateAndNormalizeAttrDecl` for the rest — so a declaration it accepts serializes to an `<!ATTLIST>` that a validating parser accepts and recovers equivalently: `elem`/`name` must be valid XML Names (parser Name production, colon allowed; no leading or trailing colon on `name`); `xml:id` must be `AttrID`; an ID attribute's default must be `#IMPLIED`/`#REQUIRED` (shared `idAttrDefaultInvalid` predicate with the validator); enumeration/NOTATION `enumValues` must be non-empty, well-formed (NMTOKENs / Names), and distinct; `#REQUIRED`/`#IMPLIED` carry no `defvalue` (checked against the value as given, before normalization) while `AttrDefaultNone`/`#FIXED` carry one (possibly empty), its characters required to be legal XML `Char`s (`isInCharacterRange`, reused from the serializer, plus an invalid-UTF-8 guard), validated against `atype` and, for enumeration/NOTATION, required to be a member of `enumValues`. The duplicate check runs BEFORE the non-identity parameter checks, matching the parser (a repeat wins with `ErrDuplicateDeclaration`, never masked by an also-invalid parameter). The token list is cloned before storing (a later caller mutation cannot corrupt it), and a non-CDATA `defvalue` is space-normalized `#x20`-only via `attrNormalizeSpace` (the same function the parser uses, NOT `strings.Fields`, which would also fold TAB/NBSP and diverge). The serializer emits the quoted `defvalue` for exactly the `AttrDefaultNone`/`#FIXED` kinds (even when empty, so a `""` default serializes as `""`).
