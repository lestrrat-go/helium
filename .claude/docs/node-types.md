...
The QName→(name, prefix) split (`AddElementDecl`/`GetElementDesc`) splits on the FIRST colon but treats a LEADING colon as part of the local name (mirrors libxml2 `xmlSplitQName3`): `:x` keys as `(name=":x", prefix="")`, distinct from the unprefixed `x` (`(name="x", prefix="")`), so a leading-colon element declaration is not a spurious redefinition of the unprefixed one (a leading colon is a legal XML 1.0 Name start character even though it is not a valid QName prefix). `AddAttributeDecl` splits its attribute `name` the same way but REJECTS a leading colon: the parser splits an attribute name on the first colon unconditionally (`strings.Cut`), so `:x` round-trips to `(name="x", prefix="")` in the parser but would key as `(name=":x", …)` here — a mismatch, so a leading-colon attribute name is an `ErrInvalidArgument`. It also rejects a TRAILING colon (`p:`), whose empty local part would key a degenerate empty-local declaration. The attribute lookup table is keyed by an `attrDeclKey{local, prefix, elem}` struct rather than a `local + ":" + prefix + ":" + elem` string, because that concatenation collides distinct triples: name `"b:a"` on element `"c:d"` (local `a`, prefix `b`, elem `c:d`) and name `"c:a:b"` on element `"d"` (local `a:b`, prefix `c`, elem `d`) both flatten to `a:b:c:d`; the struct key keeps them distinct, mirroring the parser's `specialAttrKey`.

The `DTD.Add*` builders (`AddEntity`/`AddNotation`/`AddElementDecl`/`AddAttributeDecl`) build a declaration from public parameters, register it in the lookup table, AND link it into the DTD child list so it serializes. `AddAttributeDecl(elem, name, atype, def, defvalue, enumValues)` is the `<!ATTLIST>` counterpart; the low-level table-only `registerAttribute` is unexported. `AddAttributeDecl` validates every parameter against the same rules the parser's `<!ATTLIST>` path and the declaration-time validity constraints enforce — `validateAttrDeclIdentity` for the identity parts (element name, attribute QName), then a duplicate check, then `validateAndNormalizeAttrDecl` for the rest — so a declaration it accepts serializes to an `<!ATTLIST>` that a validating parser accepts and recovers equivalently: `elem`/`name` must be valid XML Names (parser Name production, colon allowed; no leading or trailing colon on `name`); `xml:id` must be `AttrID`; an ID attribute's default must be `#IMPLIED`/`#REQUIRED` (shared `idAttrDefaultInvalid` predicate with the validator); enumeration/NOTATION `enumValues` must be non-empty, well-formed, and distinct (enumeration tokens are NMTOKENs, which permit a colon; NOTATION tokens are colon-free Names via `isValidName`, since a NotationDecl name forbids a colon — `parseNotationDecl` rejects one as "colons are forbidden from notation names"); `#REQUIRED`/`#IMPLIED` carry no `defvalue` (checked against the value as given, before normalization) while `AttrDefaultNone`/`#FIXED` carry one (possibly empty), its characters required to be legal XML `Char`s (`isInCharacterRange`, reused from the serializer, plus an invalid-UTF-8 guard) and free of a raw `&` (which does not round-trip: the serializer escapes it to `&amp;` but the parser stores the unsubstituted reference as literal `&#38;`, so the value grows on each cycle), validated against `atype` and, for enumeration/NOTATION, required to be a member of `enumValues`. A value-bearing default on a namespace-declaration attribute — `name` `"xmlns"` (the default namespace) or a `name` carrying the reserved `xmlns` prefix — additionally must bind a URI the Namespaces-in-XML reserved-prefix/URI rules permit: `checkNamespaceDeclDefault` reuses the parser's pure predicates (`reservedPrefixedNamespaceViolation`/`reservedDefaultNamespaceViolation`, shared with the start-tag path so the two cannot drift) to reject exactly the bindings the parser rejects when it applies the default on a matching element — the `xml` prefix bound to a non-XML URI, the `xmlns` prefix declared at all, or either reserved URI (XML or XMLNS) bound to another prefix or to the default namespace. An empty default never binds (so it is not checked), and an ordinary attribute whose name merely starts with `xml` (`xml:space`, `xml:lang`) is not a namespace declaration and is unaffected. The duplicate check runs BEFORE the non-identity parameter checks, matching the parser (a repeat wins with `ErrDuplicateDeclaration`, never masked by an also-invalid parameter). The token list is cloned before storing (a later caller mutation cannot corrupt it), and a non-CDATA `defvalue` is space-normalized `#x20`-only via `attrNormalizeSpace` (the same function the parser uses, NOT `strings.Fields`, which would also fold TAB/NBSP and diverge). The serializer emits the quoted `defvalue` for exactly the `AttrDefaultNone`/`#FIXED` kinds (even when empty, so a `""` default serializes as `""`).
