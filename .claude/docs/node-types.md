...
- `DeclareNamespace(prefix, uri)` / `AddNamespaceDecl(ns)` — keep AT MOST ONE declaration per prefix in `nsDefs` and NEVER touch the node's active namespace (`n.ns`) or expanded name. `DeclareNamespace` allocates a fresh `*Namespace`; `AddNamespaceDecl` attaches the caller's existing object (no alloc), so a tree-builder can reuse one Namespace as both a declaration and an element's active ns. Both apply the same rule keyed on the prefix. The CONFLICT test runs FIRST, whether or not an nsDefs entry exists: if the prefix is in use by `n.ns` or any attribute's `ns` at a URI DIFFERENT from the requested URI, that is a genuine conflict and the tree is unchanged (`DeclareNamespace` returns a `%w`-wrapped `ErrInvalidOperation`; `AddNamespaceDecl` is void so it silently declines). This covers both the fresh-append path (`SetActiveNamespace("p",X)` then `DeclareNamespace("p",Y)` with `Y≠X` — no nsDefs entry yet, but declaring `p→Y` while the reconciler synthesizes `p→X` would emit two `xmlns:p`) and the existing-entry path. A use at the SAME URI is not a conflict (the reconciler finds the prefix already bound to that URI and synthesizes nothing). When there is no conflict: (1) no existing entry → append; (2) existing entry, same URI → no-op; (3) existing entry, different URI → COLLAPSE (replace the single slot; the old object is left unmutated, so an aliasing `n.ns`/`attr.ns` or a caller-retained handle is unaffected). Prefix GENERATION for a conflicting holder is a serialize/reconcile concern (`writer.go` `reconcileNamespaces`), never in these mutators. `AddNamespaceDecl(nil)` appends nil verbatim (dedup skipped) for callers deliberately corrupting a node.
