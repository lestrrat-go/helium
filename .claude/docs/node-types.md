...
`DeclareNamespace(prefix, uri)` is idempotent per prefix: a prefix already declared with the same URI is a no-op, and with a different URI updates the single existing declaration in place, so the node holds at most one declaration per prefix and serialization never emits a duplicate or conflicting `xmlns`. `AddNamespaceDecl` is the raw append (no dedup) for tree builders reusing a `Namespace` object.
