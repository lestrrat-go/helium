...
`DeclareNamespace(prefix, uri)` is idempotent per prefix and leaves the node holding at most one declaration for the prefix: a prefix already declared with the same URI is a no-op; a different URI (or a prefix declared more than once) rebinds the first declaration to `uri` and drops every later same-prefix declaration. The first declaration is replaced with a fresh `Namespace` rather than rewriting the existing object, so a `Namespace` supplied via `AddNamespaceDecl` is never mutated. When the node's active namespace uses that prefix it is repointed to the surviving declaration, so serialization never emits a duplicate or conflicting `xmlns`. The call is rejected with an error wrapping `ErrInvalidOperation` (node left unchanged) when an attribute on the node binds the same prefix to a different URI — collapsing to one declaration would change that attribute's namespace. `AddNamespaceDecl` is the raw append (no dedup) for tree builders reusing a `Namespace` object.
