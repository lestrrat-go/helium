...
| encoding/xml shim | Marshal, Unmarshal, Encoder, Decoder, Token, struct tags | Strict-only, xmlns before regular attrs, InputOffset approximate, undeclared prefixes rejected, `version = "2.0"` (spaces around `=`) rejected where encoding/xml accepts it, a declaration violating the XMLDecl grammar (`charset=`, missing/empty version, empty encoding, non-`yes`/`no` standalone, repeated or out-of-order pseudo-attributes) rejected through BOTH `Unmarshal` and `Decoder` where encoding/xml accepts it, a declaration anywhere but the very first thing in the document (after an earlier declaration, comment, PI or doctype; only leading whitespace precedes it) likewise rejected through both where encoding/xml accepts it as an ordinary ProcInst, the reserved `xml` PI target in any casing (`<?XML?>`, `<?Xml?>`, `<?xMl?>` — `PITarget` subtracts the name case-insensitively) rejected wherever it appears through `Unmarshal`, reader-backed `Decoder` and TokenReader-backed `Decoder` alike, while a longer xml-prefixed target (`<?xmlversion?>`, `<?xml-stylesheet?>`) stays a legal ordinary PI |
