...
- DTD element-content validity (VC: Element Valid), whitespace-provenance subset: with `SubstituteEntities(true)` the DOM cannot tell a whitespace text node produced by a character reference (`&#32;`) or a general-entity reference apart from literal source whitespace, so `collectChildElements`/`validateElementContent` (`valid.go`) treat all whitespace-only text as ignorable. Three W3C xmlconf invalid cases therefore stay under-rejected (accepted): a character reference to a whitespace char in element-only content is character data, not S — invalid (`rmt-e2e-15g`); an EMPTY element whose only content is an empty entity reference `&empty;` is invalid because the reference is content (`rmt-e2e-15a`, the entity expands to nothing so the DOM element is childless); and an entity whose replacement text is a non-whitespace string is invalid (`rmt-e2e-15h`, additionally mis-expanded — helium re-parses the entity replacement `&#32;` as a char ref into a space rather than keeping the literal 5 characters). Closing these needs a parser-set node-level provenance flag (char-ref/entity-derived whitespace ≠ literal whitespace), a pervasive node-model change; left as xfail. A CDATA section is a distinct node type (provenance preserved), so a CDATA section in element-only content — even empty or whitespace-only, never S — IS rejected (`sun/invalid empty`); and an NMTOKENS/IDREFS/ENTITIES token separator is exactly `#x20` (`splitNormalizedTokens`), so a character-reference whitespace char inside a tokenized attribute value is part of the token, making e.g. `abc&#9;xyz` a single invalid NMTOKEN (`rmt-e2e-20`).
