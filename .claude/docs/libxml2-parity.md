...
| Schematron | Assert, report, let variables, name, value-of, old (ASCC) + new (ISO) syntax, `queryBinding` (XPath 1.0 and XPath 3.1 families) |
...
- Schematron `queryBinding`: helium reads the attribute and refuses a query language binding it does not implement (`ErrUnsupportedQueryBinding`, a fatal compile diagnostic), and evaluates a schema naming `xslt3`/`xpath3`/`xpath31` with XPath 3.1. libxml2 ignores the attribute entirely and evaluates every schema as XPath 1.0, so a 2.0-or-later schema silently produces XPath 1.0 answers there. The XPath 1.0 path (an absent attribute, or `xslt`/`xslt1`/`xpath`/`xpath1`) stays byte-identical to libxml2, and every libxml2-compat schematron golden is unaffected because none of the fixtures carries the attribute.
