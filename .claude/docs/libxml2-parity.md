...
| MaxNodeContentSize(int) | XML_MAX_TEXT_LENGTH (intent) | ✅ | Per-limit knob: max bytes of a single CDATA/comment/PI/char-data run (0=default `DefaultMaxNodeContentSize` 10 MiB, <0=unlimited). Fires during accumulation; over-cap → `ErrNodeContentTooLarge`. Streaming SAX (`CharBufferSize>0`) char data is exempt (already chunked) |
