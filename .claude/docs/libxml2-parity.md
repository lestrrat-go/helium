...
- Writer invalid-character default: the `Writer` REJECTS a character invalid in the target XML version (`ErrInvalidXMLChar` / SERE0006) by default, where libxml2 silently replaces it with `&#xFFFD;`. `Writer.RejectInvalidChars(false)` restores libxml2-parity replacement — in text/attribute values `&#xFFFD;` under `EscapeNonASCII`/US-ASCII output, raw U+FFFD otherwise; in the reference-less contexts (comment text, PI data, CDATA-section content, and DTD literals) always raw U+FFFD, since those contexts admit no character reference — which also makes an XML 1.1 RestrictedChar invalid THERE (including the C1 half `#x7F-#x84`/`#x86-#x9F`, a legal literal 1.0 Char) even though text/attribute values carry it as a decimal reference; NEL `#x85` and LS `#x2028` are not RestrictedChar and stay legal literally. The policy applies in EVERY serialization context except the character-map exemption, not only text/attribute values: it also covers every verbatim NAME (DOCTYPE/`<!ENTITY>`/`<!NOTATION>`/`<!ELEMENT>`/`<!ATTLIST>`/content-model/NDATA/element/attribute/entity-reference names — validated against the XML Name grammar, so an invalid name is REJECTED in both modes with `ErrWriterInvalidName`, having no U+FFFD form) and numeric character-reference TARGETS (`CreateCharRef("#N")` and `&#N;` in an entity value — validated version-sensitively via `isSerializableChar`, so `&#1;` is invalid for XML 1.0 output but legal for XML 1.1). A character map is exempt: the replacement string is emitted verbatim (Serialization 3.1 §7). A parsed document never carries such a character or name (the parser rejects them), so every serialization golden is byte-identical under either setting; the divergence only surfaces for a caller who builds a tree with an out-of-range character or invalid name in memory.
