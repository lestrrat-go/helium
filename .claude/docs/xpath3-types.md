...
`fn:normalize-space` reuses the SAME interleaved content-kind check: its arity-1 signature is `item()?` (NOT `xs:string?`) so the node argument reaches `fnNormalizeSpace` UN-atomized, and the function performs the `xs:string?` function-conversion atomization itself via `coerceArgToStringCont` (`functions.go`) → `coerceAtomizedStringCont`, which threads `contentKindProvider(ctx)` through `atomizeStreamCont` — the identical array-aware, encounter-order path fn:data uses. So an element-only-typed node argument raises `FOTY0012` whether passed directly or nested inside an array (`normalize-space([/*])`), and atomization precedes the cardinality check. A nil provider (the common non-schema-aware case) makes `coerceAtomizedStringCont` byte-identical to `coerceAtomizedString`, so no other `xs:string?`-arg function is affected.
