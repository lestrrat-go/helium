...
`fn:normalize-space` reuses the same content-kind check via `checkArgContentKind` (`functions_node.go`): its arity-1 signature is `item()?` (NOT `xs:string?`) so the node argument reaches `fnNormalizeSpace` UN-atomized, the function runs `checkContentKindItem` on each argument item (element-only content ⇒ `FOTY0012`), then performs the `xs:string?` function-conversion atomization itself via `coerceArgToString`. `checkArgContentKind` propagates only the `FOTY0012` error (the empty-content skip result is ignored) and is a no-op without a `ContentTypeKindProvider`.
