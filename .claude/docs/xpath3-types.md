...
- `UnionMembers` → per-member atomization metadata (built-in base + list-item info), precomputed from `SchemaDeclarations` in `nodeItemFor`; `ActiveUnionMember` → index of the value-dependent ACTIVE member (the first member the value FULLY validates against — lexical/value cast AND facets/list-length via `SchemaDeclarations.ValidateCastWithNS`, matching the `$value` selection), or -1. A list active member expands to per-item atoms; an atomic active member yields a single atom typed as that member
...
`resolveQNameFromNode` predeclares the `xml` prefix (→ the XML namespace) without requiring a binding on any node, matching `xsd.resolveLexicalQName`, so an xs:QName value such as `xml:lang`/`xml:space` atomizes correctly. List-typed node atomization (`atomizeListToken`, used by both `atomizeStream` and the value-comparison atom iterator) splits whitespace tokens and, when the item type's built-in base is `xs:QName`/`xs:NOTATION`, resolves each token against the node's in-scope namespaces (preserving the user/list-item type name and its built-in base) rather than the context-free `CastFromString`, so a list of xs:QName atomizes in a schema-aware assertion. A UNION-typed node is atomized through its precomputed ACTIVE member (`ActiveUnionMember`, resolved by `resolveActiveUnionMember` in `nodeItemFor` with FULL schema-aware validation — cast validity AND facets/list-length/assertions via `SchemaDeclarations.ValidateCastWithNS`, the ctx threaded through `nodeItemFor`): `atomizeUnionItems` (used by both the stream and comparison paths) expands a LIST active member to per-item atoms and yields an ATOMIC active member as a single atom typed as that member. Because selection is full validation, a member that casts but fails its own facets (e.g. an xs:list with `xs:length=2` against `"1 2 3"`) is correctly skipped — matching the `$value` path. All of this only activates when `UnionMembers`/`ListItemType` are populated (schema-aware atomization), so non-schema-aware xpath3/xslt3 behavior is unchanged.
