...
- `--query-binding NAME` forces the query language binding (`xslt`, `xslt1`, `xpath`, `xpath1`, `xslt3`, `xpath3`, `xpath31`), overriding the schema's `queryBinding` attribute; an unrecognized name is rejected before compilation. Unset, the attribute decides, and an unsupported attribute value exits `ExitSchemaComp`
