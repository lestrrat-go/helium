...
- `--huge` → lifts the tunable parser limits for trusted input: `MaxNameLength(-1)`, `MaxEntityAmplification(-1)`, `MaxContentModelDepth(-1)`, `MaxNodeContentSize(-1)` (disables the 10 MiB single-construct CDATA/comment/PI/char-data cap), and `MaxDepth(0)`. An explicit `--max-depth` still wins (applied after `--huge`).
