...
**XML EBCDIC over a reader**: EBCDIC is not ASCII-compatible and its detection/decode (`parser_document.go`) replays the original raw bytes from `pctx.rawInput`, which the streaming reader path does not retain. `ParseReader` therefore wraps the reader in a `bufio.Reader` and peeks the first 4 bytes for the EBCDIC invariant prefix (`0x4C 0x6F 0xA7 0x94`). When present it reads the whole input and routes through `Parse([]byte)` (which sets `rawInput`), so an EBCDIC document parses identically via `ParseReader`/`ParseFile` and `Parse([]byte)`. `bufio.Peek` leaves the bytes and any error returned alongside them in place, so the common (non-EBCDIC) case still streams unchanged and a reader that returns data plus a non-EOF error on the same read still surfaces that error.
