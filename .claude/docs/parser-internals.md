...
calls `checkBOMEncodingConflict` (`parser_encoding.go`). It canonicalizes the
declared EncName through the SAME `internal/encoding` resolver the parser uses —
`encoding.UnicodeBOMFamily` (which consults `Load`), returning `utf-8`,
`utf-16be`, `utf-16le`, `utf-16` (generic, either endian), or `""` (non-Unicode
or unresolved) — so every alias `Load` accepts is handled with NO parallel table.
A declared name that resolves to the same family as the BOM is compatible (a
generic `utf-16` matches EITHER UTF-16 BOM, since the BOM fixes endianness); an
unresolvable name is left to `switchEncoding` and not newly rejected; anything
else — a different family, the opposite-endian UTF-16, or a non-Unicode encoding
like `iso-8859-1` — is a fatal `ErrEncodingBOMMismatch` (XML §4.3.3; W3C not-wf
`hst-lhs-007`/`hst-lhs-008`). This is a byte-for-byte stricter stance than
libxml2, which downgrades the mismatch to a warning and continues. Only a
consumed BOM sets `autoEncoding`, so
...
- `parser_element.go` owns recursive element parsing, start/end tags, attributes, and character data. `parseStartTag` enforces XML §3.1 P40/P44 required inter-attribute whitespace: after EVERY attribute — the default-namespace, prefixed-namespace, and regular-attribute branches alike — the next character must close the tag (`>` or `/>`) or be whitespace; a NameStartChar with no intervening `S` is a fatal `ErrSpaceRequired` (mirrors libxml2's uniform `next_attr` check; W3C not-wf `attlist10`/`attlist11`, `o-p40fail1`/`o-p44fail4`, `not-wf-sa-186`). `parseStartTag` also enforces the Namespaces in XML §3 well-formedness constraints on prefix binding and reserved namespace names: (1) every attribute prefix must resolve to an in-scope namespace — the reserved `xml` prefix is always bound (via `lookupNamespace`), any other unbound prefix is a fatal `namespaceError` ("namespace '<pfx>' not found"), the same rule the element-QName resolution applies (W3C not-wf `rmt-ns10-026`); and (2) a DEFAULT namespace declaration (`xmlns="uri"`) may not bind either reserved URI — `validateDefaultNamespaceDecl` rejects `http://www.w3.org/XML/1998/namespace` and `http://www.w3.org/2000/xmlns/` as a fatal `namespaceError` (W3C not-wf `rmt-ns-e1.0-13a`/`rmt-ns-e1.0-13b`). Both checks apply to literal and DTD-defaulted declarations, mirror libxml2's `xmlParseStartTag2` reserved-URI and undefined-prefix checks, and leave `xmlns=""` (a legal default-namespace undeclaration) and the reserved `xml`/`xmlns` PREFIX branches (handled by `validatePrefixedNamespaceDecl`) untouched. The colon-in-Name / `xmlns` element-prefix boundary (`rmt-ns-e1.0-13c` and similar) is a separate name-tokenizer concern and is NOT enforced here.
...
3. Parse entity content if needed (`parseBalancedChunkInternal`, or `parseExternalEntityPrivate` for external entities)
   ...
   - An internal general entity's replacement text must be well balanced w.r.t. element nesting (WFC: parsed entities must be well-formed, XML §4.3.2). `parseContent` stops at a `</` that would close an element opened OUTSIDE the chunk (the pseudo-root), so `parseBalancedChunkInternal` treats a NON-exhausted cursor after `parseContent` as a fatal `ErrEntityNotWellBalanced` — the entity opens with, or crosses, an element boundary (`<!ENTITY e "</foo><foo>">` referenced in content; W3C not-wf-sa-074). This is a well-formedness check applied in every parse mode.
...
**Character-reference provenance (element-content validity).** Whitespace produced by a character reference does NOT match the S nonterminal, so it is not ignorable in element-only content (XML §3.2.1, errata 2e E15) even though `SubstituteEntities(true)` makes the resulting Text node byte-identical to one from literal whitespace. `parseReference` (`parser_entity_ref.go`) sets the transient `pctx.charDataFromCharRef` flag around a `&#N;`/`&#xN;` delivery; `TreeBuilder.Characters` stamps the resulting Text node's `fromCharRef` field from it (sticky — a later literal append into the same merged node does not clear it). The distinction survives general-entity expansion: an entity's replacement text is re-parsed by `parseBalancedChunkInternal` (which inherits the `sax`/`treeBuilder`), so an entity value of `&#32;` (E15h) marks the entity's cached Text child, while an entity value that is a literal space (E15e) or a `&#32;` expanded at DECLARATION time (E15f, replacement text = `" "`) does not; `replayEntityNode` propagates the cached child's `fromCharRef` onto the replayed content node. `parseReference` also sets `pctx.elem.contentHasReference` for ANY content reference so element-content validity can reject a reference inside an EMPTY element (E15a) even when it expands to nothing. See `.claude/docs/node-types.md` for the node fields and `validation-pipeline.md`/`valid.go` for the read side.
