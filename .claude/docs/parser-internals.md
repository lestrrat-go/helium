...
- `maxAmpl int` — max amplification factor (5 default, 0 with `MaxEntityAmplification(-1)`); resolved from `parserConfig.maxEntityAmpl` via `resolveLimit`
...
2. `entityCheck(ent, size)` — amplification guard
   ...
   - Max amplification: 5× input (tunable via `MaxEntityAmplification(int)`; disabled with `MaxEntityAmplification(-1)`)
