...
Under `SubstituteEntities(false)`, a general entity referenced from an attribute value is additionally checked against the XML 1.0 attribute-value WFCs on its TRANSITIVE replacement text — "No External Entity References" and "No < in Attribute Values" — by `parseAttributeValueInternal`'s `attributeEntityWFC` predicate (`parser_element.go`). It walks nested `&name;` references via `pctx.getEntity` (a table lookup that fires NO SAX callbacks — unlike `decodeEntities`), with a visited-set cycle guard, and classifies the entity as `attrWFCExternal`/`attrWFCUnparsed`/`attrWFCLessThan`/`attrWFCNone` (`entity.go`). The result is NOT memoized: an attribute default value parsed mid-DTD may reference a nested entity declared LATER, so a classification computed against the still-incomplete entity tables must never be cached; the walk is side-effect-free and cheap, so it re-runs and always reflects the current tables. The check runs on EVERY attribute occurrence, INDEPENDENT of `ent.checked`: the `checked` bit records only the weaker general-content (element) check, so an entity first expanded in element content must still be re-validated under the stricter attribute rules. The DIRECT case (the referenced entity itself external/unparsed, or its own content directly containing `<`) is rejected earlier by `parseEntityRef`; predefined entities (`&lt;` etc.) are never flagged. The separate `decodeEntities` "Entity Declared" validation (undefined nested entity, W3C not-wf-sa-077) stays once-only-memoized so its `ResolveEntity` SAX events are unchanged.
