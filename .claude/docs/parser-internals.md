...
KNOWN gap (orthogonal): a parameter-entity REFERENCE that expands INSIDE or immediately ADJACENT to a markup declaration in the external subset — e.g. `<!ATTLIST doc a1 CDATA%e;>`, `<!ATTLIST%e;a1 ...>`, or `<!ELEMENT doc (#PCDATA %e;)>` — is not expanded (PE recognition happens only at blank-adjacent positions via `skipBlanks`, not at every point the external-subset grammar allows, and the §4.4.8 leading/trailing-space padding of an included PE is not applied). Affects W3C not-sa-019/020/021.
...
Under `SubstituteEntities(false)`, a general entity referenced from an attribute value is additionally checked against the XML 1.0 attribute-value WFCs on its TRANSITIVE replacement text — "No External Entity References" and "No < in Attribute Values" — by `parseAttributeValueInternal`'s `attributeEntityWFC` predicate (`parser_element.go`). `scanAttrEntityWFC` walks nested `&name;` references via `pctx.getEntity` (a table lookup that fires NO SAX callbacks — unlike `decodeEntities`) using an EXPLICIT work stack rather than native recursion (so a long acyclic internal-entity chain cannot grow the Go call stack), with a visited-set that both guards reference cycles and bounds the walk to the distinct declared entities, and classifies the entity as `attrWFCExternal`/`attrWFCUnparsed`/`attrWFCLessThan`/`attrWFCNone` (`entity.go`). The result is NOT memoized: an attribute default value parsed mid-DTD may reference a nested entity declared LATER, so a classification computed against the still-incomplete entity tables must never be cached; the walk is side-effect-free and cheap, so it re-runs and always reflects the current tables. The check runs on EVERY attribute occurrence, INDEPENDENT of `ent.checked`: the `checked` bit records only the weaker general-content (element) check, so an entity first expanded in element content must still be re-validated under the stricter attribute rules. The DIRECT case (the referenced entity itself external/unparsed, or its own content directly containing `<`) is rejected earlier by `parseEntityRef`; predefined entities (`&lt;` etc.) are never flagged. The separate `decodeEntities` "Entity Declared" validation (undefined nested entity, W3C not-wf-sa-077) stays once-only-memoized so its `ResolveEntity` SAX events are unchanged. A DTD **attribute default value** is parsed while the DTD is still being read, so a nested entity it references may be forward-declared; `validateAttributeDefaultsWFC` (`parser_element.go`, run from `parseDocument` once the internal+external subset are fully parsed) re-scans every stored default value with the entity tables complete, enforcing the same WFCs on the default-value DECLARATION (W3C rmt-e3e-12) regardless of whether any element uses the default.
