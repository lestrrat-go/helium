...
- Restricted characters — raw literal values reject in text, attributes, comments, PIs, CDATA, non-PUBLIC DTD literals, and external-DTD `IGNORE` sections via `parser_content.go` `isLiteralCharValue`, `parser_dtd_attr.go` `scanQuotedLiteral`, and `parser_dtd_subset.go` `parseConditionalSections`; `IGNORE` scans decode non-delimiter UTF-8 as runes before validation. Character references, including stored entity values, accept their XML 1.1 values via `parser_entity_ref.go` `parseCharRef` / `parseStringCharRef` / `isXML11CharValue`
