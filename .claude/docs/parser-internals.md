...
- Restricted characters — raw literal values reject in text, attributes, comments, PIs, CDATA, non-PUBLIC DTD literals, and external-DTD `IGNORE` sections via `parser_content.go` `isLiteralCharValue`, `parser_dtd_attr.go` `scanQuotedLiteral`, and `parser_dtd_subset.go` `parseConditionalSections`; character references, including stored entity values, accept their XML 1.1 values via `parser_entity_ref.go` `parseCharRef` / `parseStringCharRef` / `isXML11CharValue`
...
- Entity sub-parsers seed nested context via `inheritNestedParserState` (`parser_entity_decl.go`) — copies policy + running resource-accounting (depth, `ebcdicConsumed`); an external parsed entity inherits document XML version unless its TextDecl supplies a compatible version; see its doc comment
