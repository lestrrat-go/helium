...
- Writer structural-serialization sentinels (`errors.go`) — each flags a DOM the writer cannot serialize into well-formed XML, wrapped (via `%w`) into a descriptive value-bearing message so a caller can branch with `errors.Is`: `ErrWriterReservedElementName` / `ErrWriterReservedAttributeName` / `ErrWriterReservedNamespacePrefix` (an `xmlns` name or prefix that must go through `DeclareNamespace`, not a literal name), `ErrWriterInvalidElementName` / `ErrWriterInvalidAttributeName` / `ErrWriterInvalidNamespacePrefix` (a name that is not a valid QName/NCName and would inject raw markup), `ErrWriterUnboundNamespacePrefix` (an element or attribute QName whose non-empty prefix resolves to an empty namespace URI, e.g. an `html.Parse` colon name built via `CreateNamespace(prefix, "")` — emitting `prefix:local` with no `xmlns:prefix` yields output the parser rejects with `namespace 'prefix' not found`; checked by `checkNamespaceBinding` on the element and attribute paths of both the generic (`writeNode`) and XHTML (`dumpXHTMLNode`/`dumpXHTMLAttrList`) serializers; the reserved `xml` prefix is exempt — it is implicitly bound to the XML namespace and reparses without a declaration, so it serializes even when its namespace object carries an empty href), `ErrWriterInvalidComment` (comment content containing `--` or ending `-`), `ErrWriterInvalidPITarget` / `ErrWriterInvalidPIContent` (a bad PI target, or PI content containing `?>`), and `ErrWriterInvalidDTDNode` (a DTD node that cannot serialize into well-formed XML: an element-content/entity/declaration node whose type field holds an unrecognized value, an empty DOCTYPE name, a public identifier carrying a non-`PubidChar`, or a DOCTYPE system literal containing both quote characters — `checkPubid` covers DOCTYPE and notation public identifiers, while `checkSystemLiteral` rejects both quote characters only for DOCTYPE system identifiers). The name/comment/PI guards record the sticky error and preserve an earlier I/O error
