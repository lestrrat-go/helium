...
- `node.DeclareNamespace(prefix, uri) → error` / `node.AddNamespaceDecl(*Namespace)` (promoted to `*Element` etc.) — collapse to AT MOST ONE declaration per prefix in `nsDefs` and never change the node's active namespace/expanded name. `DeclareNamespace` allocates a fresh `*Namespace`; `AddNamespaceDecl` attaches the caller's existing one WITHOUT allocating (a built tree can reuse one `Namespace` as both a declaration and an element's active namespace). A prefix in use by the element's own name or an attribute at a URI DIFFERENT from the requested URI is a genuine conflict (checked first, whether or not an nsDefs entry exists — this covers the fresh-append path where the active ns/attr set the prefix but no nsDefs entry does): the tree is left unchanged and `DeclareNamespace` returns a `%w`-wrapped `ErrInvalidOperation` (`AddNamespaceDecl` is void, so it silently declines). Otherwise same-prefix+same-URI is a no-op and a different-URI redeclare COLLAPSES the slot (replacing it with a fresh object, leaving any caller-retained handle unmutated). Real rebinders remove the prefix first (`RemoveNamespaceByPrefix`); caller owns any ns passed to `AddNamespaceDecl` and must not share it across independently-mutated nodes
