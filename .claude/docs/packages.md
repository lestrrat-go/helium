...
- **NewParser() → Parser** — create fluent builder for XML parsing (clone-on-write value type)
  ...
  - `Parser.ErrorHandler(ErrorHandler)` — sets the handler for validation errors produced during DTD validation (`ValidateDTD`); individual errors are delivered as they occur and `Parse` returns `ErrDTDValidationFailed` on failure. If the handler is an `io.Closer`, it is closed when `Parse` returns
...
- **NewCompiler() → Compiler** — create fluent builder for grammar compilation
  ...
  - `Compiler.FS(fs.FS)` — sets the `fs.FS` used to load schemas referenced by `include` and `externalRef`. A nil value restores the default permissive FS (`internal/iofs.PermissiveRoot`, opens via `os.Open`). Names are built with `filepath.Join` from `BaseDir` + the include href, so they may be absolute / OS-style; FS implementations enforcing `fs.ValidPath` (`os.DirFS`, `fstest.MapFS`) reject them, so a sandboxing FS must accept OS-style names
