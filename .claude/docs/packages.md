...
- `node.DeclareNamespace(prefix, uri) → error` (promoted to `*Element` etc.) — declares a namespace on the node (libxml2 `xmlNewNs`) WITHOUT making it the active namespace. Idempotent per prefix: an existing prefix redeclared with the same URI is a no-op, with a different URI updates the single existing declaration in place. The node holds at most one declaration per prefix, so serialization never emits a duplicate or conflicting `xmlns` for one prefix
