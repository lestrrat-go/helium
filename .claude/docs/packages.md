...
- `ErrUnsupportedOutputEncoding` — sentinel returned by the writer when an explicitly set `OutputEncoding` names an encoding it cannot emit (not UTF-8/US-ASCII and not loadable by the internal encoder table); scoped to the override so a document's own unloadable parsed encoding stays declaration-only (default output byte-identical); match with `errors.Is`
