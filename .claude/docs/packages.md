...
- `CopyDoc(src) → *Document` / `CopyDTDInfo(src, dst)` / `CopyExtSubset(src, dst)` — document-level deep copy: `CopyDoc` copies the whole tree; `CopyDTDInfo` copies internal-subset metadata + entities/elements/attributes/notations; `CopyExtSubset` gives `dst` its own independent deep copy of the source's EXTERNAL DTD subset (`copy.go` / `dtd.go`)
