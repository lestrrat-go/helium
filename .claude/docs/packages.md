...
- Adaptive serialization delegates a singleton element/document item to the XML method. Comment and processing-instruction items, including top-level primary `xsl:comment` and `xsl:processing-instruction` output, use the XML writer without a declaration, preserving markup and target-version character validation without character maps or normalization. A primary top-level comment/processing-instruction sequence keeps its item separator outside the DOM, so standalone adaptive serialization places the separator between items without switching to the XML fallback. A quoted string-like atomic delegates character-data processing to the Text method before adaptive quoting, including normalization and character maps.
