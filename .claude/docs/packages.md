...
- `Document.GetElementByID(id)` — O(1) via hash table, O(n) fallback; nil if no match. In a valid document an ID is unique, so at most one match. If several elements share an ID (invalid XML) the table path returns the last-registered element (last in document order) and the fallback walk the first — don't rely on either
