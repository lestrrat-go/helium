...
- DTD declaration builders — the `DTD.Add*` family builds a declaration from public parameters, registers it in the lookup table, AND links it into the DTD child list so it serializes: `AddEntity(name, typ, publicID, systemID, content) → (*Entity, error)`, `AddNotation(name, publicID, systemID) → (*Notation, error)`, `AddElementDecl(name, typ, content) → (*ElementDecl, error)`, and `AddAttributeDecl(elem, name, atype, def, defvalue, enumValues Enumeration) → (*AttributeDecl, error)` — the latter builds an `<!ATTLIST>` declaration: `name` may be a QName (its prefix is split off on the first colon, as `AddElementDecl` does), `atype`/`def` are `enum.Attr*`/`enum.AttrDefault*`, `defvalue` is validated against `atype` when non-empty, and `enumValues` is required for `enum.AttrEnumeration`/`enum.AttrNotation` (rejected empty, else the serialized `()` would not re-parse). A collision with an existing declaration of the same kind+name returns an error wrapping `ErrDuplicateDeclaration`. The low-level `registerAttribute` (table-only, no child-list link) is unexported; callers use `AddAttributeDecl` (`dtd.go`)
