...
- `node.DeclareNamespace(prefix, uri) → error` (promoted to `*Element` etc.) — declares a namespace on the node (libxml2 `xmlNewNs`) WITHOUT making it the active namespace. Idempotent per prefix and holds at most one declaration per prefix: same URI is a no-op; a different URI (or a prefix declared more than once) rebinds the first declaration and drops every later same-prefix one, replacing the first slot with a fresh `Namespace` (never mutating a caller's `AddNamespaceDecl` object) and repointing the active namespace when it shares the prefix, so serialization never emits a duplicate or conflicting `xmlns`. Rejected with an error wrapping `ErrInvalidOperation` (node unchanged) when an attribute on the node binds the same prefix to a different URI
- `node.AddNamespaceDecl(*Namespace)` (promoted to `*Element` etc.) — appends an existing `*Namespace` to the node's declarations WITHOUT allocating a new one (unlike `DeclareNamespace`, and WITHOUT the per-prefix dedup), so a built tree can reuse one `Namespace` as both a declaration and an element's active namespace; caller owns the ns and must not share it across independently-mutated nodes
