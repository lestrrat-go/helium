...
- `ErrUnsupportedOutputEncoding` — sentinel returned by the writer for an explicitly set `OutputEncoding` that cannot be emitted faithfully, in two cases: (1) the encoding is not UTF-8/US-ASCII and not loadable by the internal encoder table; (2) the encoding is US-ASCII (any alias) and a non-ASCII character appears where no character reference is permitted (comment, CDATA, PI, element/attribute name, namespace prefix, notation name — text and attribute values stay char-referenced). Scoped to the override so a document's own unloadable parsed encoding stays declaration-only (default output byte-identical); match with `errors.Is`
