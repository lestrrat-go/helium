...
- `ErrInvalidArgument` — sentinel returned when a public builder is given an argument that cannot produce a well-formed result (e.g. an `AddAttributeDecl` element/attribute name that is not a valid XML Name, a leading-colon attribute name, an out-of-range attribute type or default kind, a malformed/duplicate enumeration or NOTATION token, or a default value illegal for the type or not a declared enumeration member). Wrapped (via `%w`) into a message describing the violation; match with `errors.Is`
...
- DTD declaration builders — the `DTD.Add*` family builds a declaration from public parameters, registers it in the lookup table, AND links it into the DTD child list so it serializes: `AddEntity(name, typ, publicID, systemID, content) → (*Entity, error)`, `AddNotation(name, publicID, systemID) → (*Notation, error)`, `AddElementDecl(name, typ, content) → (*ElementDecl, error)`, and `AddAttributeDecl(elem, name, atype, def, defvalue, enumValues Enumeration) → (*AttributeDecl, error)` — the latter builds an `<!ATTLIST>` declaration and validates every parameter against the parser's `<!ATTLIST>` rules (`validateAttributeDeclParams`), so a declaration it accepts serializes to markup a validating parser accepts and recovers equivalently: `elem`/`name` must be valid XML Names (colon allowed, no leading colon on `name`, which is split into prefix+local on the first colon as `AddElementDecl` does); `xml:id` must be `AttrID`; `enumValues` must be a non-empty list of well-formed, distinct tokens for `enum.AttrEnumeration`/`enum.AttrNotation` (and is cloned before storing); `#REQUIRED`/`#IMPLIED` carry no `defvalue` while `AttrDefaultNone`/`#FIXED` carry one (possibly empty) validated against `atype` and, for enumeration/NOTATION, required to be a declared member. Bad arguments return an error wrapping `ErrInvalidArgument`; a collision with an existing declaration of the same kind+name wraps `ErrDuplicateDeclaration`. The low-level `registerAttribute` (table-only, no child-list link) is unexported; callers use `AddAttributeDecl` (`dtd.go`)
