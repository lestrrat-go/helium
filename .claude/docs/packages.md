...
- **NewSigner() → Signer** — create fluent builder for signing (clone-on-write value type)
  ...
  - Returned-node lifetime: the detached `ds:Signature` from `SignDetached`/`SignEnveloping` is built from the passed-in `doc`'s slab storage (`buildSignatureSkeleton` uses `doc.CreateElement`), so it is owned by `doc` and valid only while `doc` is alive and un-`Free()`d — `doc.Free()` while the caller still holds the Signature recycles the chunks backing it and corrupts it. `Document.Free` only skips recycling once a slab-backed node has been moved into another document (`slabEscaped`); a returned-but-never-placed Signature never sets that. Callers keep it past `doc.Free()` by first placing it into a destination document (e.g. `dest.DocumentElement().AddChild(sig)`), which sets `slabEscaped` and makes the later `Free()` a safe no-op. The `SignDetached`/`SignEnveloping` godoc states this.
