...
func processRetrievalMethod(ctx context.Context, budget *verifyBudget, cfg *verifierConfig, doc *helium.Document, rm *helium.Element, data *KeyInfoData, visited map[string]struct{}, depth int) error {
	...
	if len(steps) == 0 {
		return interpretRetrievalElement(ctx, budget, target, typ, data)
	}
...
func interpretRetrievalOctets(ctx context.Context, budget *verifyBudget, cfg *verifierConfig, octets []byte, typ string, data *KeyInfoData) error {
	...
	case TypeX509Data:
		...
		return parseX509Data(ctx, budget, root, data)
	default:
		return fmt.Errorf("%w: unsupported RetrievalMethod Type %q", ErrInvalidKeyInfo, typ)
	}
}
...
func interpretRetrievalElement(ctx context.Context, budget *verifyBudget, target *helium.Element, typ string, data *KeyInfoData) error {
	switch typ {
	case TypeX509Data:
