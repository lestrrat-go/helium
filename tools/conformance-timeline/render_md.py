...
def render_svg(data, path):
    ...
    # The legend wraps to as many rows as the suite labels need, and every panel below
    # shifts down by the extra rows so nothing collides -- 8 suites overflow a single row.
    def _item_w(key):
        return 34 + 7.2 * len(SHORT[key]) + 22
    rows_used, lx = 1, PL
    for s in suites:
        w = _item_w(s["key"])
        if lx + w > W - PR and lx > PL:
            rows_used += 1
            lx = PL
        lx += w
    off = (rows_used - 1) * 18
    ...
    out.append(f'<rect class="bg" x="0" y="0" width="{W}" height="{Hh}"/>')
    out.append(f'<text class="ttl" x="{PL}" y="20">Share of each suite\'s applicable cases passed, by release</text>')

    # legend (wraps to multiple rows when the labels overflow the plot width)
    lx, ly = PL, LEG_Y
    for s in suites:
        c = COLORS[s["key"]]
        label = SHORT[s["key"]]
        ...
        if lx + w > W - PR and lx > PL:
            ly += 18
            lx = PL
        out.append(f'<line x1="{lx}" y1="{ly}" x2="{lx + 22}" y2="{ly}" stroke="{c}" stroke-width="3" '
                   f'stroke-linecap="round"/>')
        out.append(f'<circle cx="{lx + 11}" cy="{ly}" r="3.4" fill="{c}"/>')
        out.append(f'<text class="leg" x="{lx + 29}" y="{ly + 4}">{label}</text>')
        lx += w
    ...
    out.append(f'<text class="sub" x="{PL}" y="{Hh - 8}">'
...
def render_md(data, path, svg_rel):
    ...
    untagged = {r["tag"] for r in rows if r.get("untagged")}
    L = []
    ...
    for t in tags:
        ...
        tag_cell = f"`{t}`" + (" ⚑" if t in untagged else "")
        L.append(f"| {tag_cell} | {data['dates'].get(t,'')} | " + " | ".join(cells) + " |")
    L.append("")
    L.append("✗ hang/OOM crasher · ⊘ in-scope case never run · ⚠ documented expected failure "
             "— **all three count as not-passing**.\n")
    if untagged:
        L.append("⚑ measured at an **untagged release candidate** (before its tag existed, e.g. by the "
                 "release gate); a re-measure at the real tag supersedes it.\n")
