...
func TestInternalSubset(t *testing.T) {
	...
	// DTD construction and its accessor methods.
	...
	// InternalSubset and CreateInternalSubset error branches: no subset, and
	// double creation.
	...
	// The DTD is inserted before an already-present root element (the non-append
	// branch).
	t.Run("created before an existing root", func(t *testing.T) {
		doc := helium.NewDocument("1.0", "UTF-8", helium.StandaloneImplicitNo)
...
func TestDTDDeclarations(t *testing.T) {
	t.Parallel()

	// AddEntity/AddNotation/ForEachEntity and the resulting Entity/Notation
	// accessor methods.
	t.Run("entities and notations", func(t *testing.T) {
		doc := helium.NewDocument("1.0", "UTF-8", helium.StandaloneImplicitNo)
	...
	// AddNotation rejects a colon-bearing
	...
	// validating parser.
	t.Run("notation name rejects a colon", func(t *testing.T) {
		doc := helium.NewDocument("1.0", "UTF-8", helium.StandaloneImplicitNo)
		dtd, err := doc.CreateInternalSubset("doc", "", "")
		require.NoError(t, err)
	...
	// DTD element/attribute declaration accessors, by parsing a DTD that declares
	// both.
	t.Run("element and attribute declaration accessors", func(t *testing.T) {
	...
	t.Run("node-interface wrappers", func(t *testing.T) {
...
func TestDTDElement(t *testing.T) {
	t.Parallel()
	...
	// IsMixedElement across the declared element content types and the not-found
	// error.
	t.Run("is mixed element by declared type", func(t *testing.T) {
		const src = `<?xml version="1.0"?>
<!DOCTYPE doc [
  <!ELEMENT doc (child)>
...
<doc><child/></doc>`
	...
	// IsMixedElement returns the same
	...
	t.Run("is mixed element across subsets", func(t *testing.T) {
...
<r><child/></r>`
	...
	// The ElementType Stringer, including the out-of-range fallback.
	t.Run("element type string", func(t *testing.T) {
